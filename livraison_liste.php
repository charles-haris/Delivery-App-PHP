...
$bdd = connect();
if (isset($_GET['supp'])){
	$requ=$bdd->query("UPDATE livraison set Annuler='oui' WHERE IDCOLIS='".$_GET['IDCOLIS']."' ");
	
  }
  
  if (isset($_GET['retir'])){
    $requ=$bdd->query("UPDATE livraison SET IDLIV=null WHERE IDCOLIS='".$_GET['IDCOLIS']."' ");
    $requ2=$bdd->query("DELETE FROM Temps WHERE IDCOLIS='".$_GET['IDCOLIS']."' ");
    }
...
<html lang="en">
  <head>
    ...
    <meta http-equiv="Refresh" content="60"; url="livraison_liste.php">
    <!-- 
      ...
      <div class="templatemo-content col-1 light-gray-bg">
        ...
        <div class="templatemo-content-container">
          <div class="templatemo-content-widget white-bg">
            <center><h2 class="margin-bottom-10">Liste des livraisons</h2></center>
    </div>
        ...
        $total=$con->prepare("SELECT COUNT(*) as totaux FROM  livraison where Statut='Non Livré' and Annuler='non' and IDLIV is not null");
        $total->execute();
        $t=$total->fetch();
        $nbr=(int)$t['totaux'];
        ...
        $rete=$con->prepare("SELECT IDCOLIS,nom,typescolis,poids,volume,adresseOrig,adresseLivraison,nbColis,Statut,Description,livraison.TypeVehicule,IDLIV FROM livraison,client where livraison.IDCLI=client.IDCLI  and Statut='Non Livré' and Annuler='non' and livraison.IDLIV is not null limit :premier, :parpage");
              ...
              <table class="table table-hover mt-3 table-striped table-bordered templatemo-user-table text-center " id="myTable">
                  <thead>
                    <tr>
                      ...
                      <td><a href="" class="white-text templatemo-sort-by">Code livreur<span class="caret"></span></a></td>
                  ...
                  <tbody>
                    ...
                    <tr>
                        ...
                        <?php 
                         $rep=$con->query("SELECT * FROM livraison where  IDCOLIS=".$id." and  IDLIV is null ",PDO::FETCH_BOUND);
                         $n=$rep->rowCount();
                         
                        ?>
                        <td><a href="livraison_liste.php?IDCOLIS=<?php  echo $id;?>&retir=ok" class="templatemo-edit-btn" >Retirer</a></td>

                        <td><a href="livraison_liste.php?IDCOLIS=<?php  echo $id;?>&supp=ok" class="templatemo-edit-btn btn-danger" >Annuler</a></td>
                    </tr>
                    <?php  }?>
                  </tbody>
                </table>  
